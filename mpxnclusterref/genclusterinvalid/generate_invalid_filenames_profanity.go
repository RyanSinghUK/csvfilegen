...
const (
	invalidSpecialChars = `<>:"/\|?*` + "`" + `~!@#$%^&+=[]{};'`
	sillyProfanity      = "sillypoop fartface doofus knucklehead"
	whitespace          = " "
)
...
func generateInvalidFilename() string {
	choices := []func() string{
		...
		generateSillyProfanityFilename,
	}
...
func generateSillyProfanityFilename() string {
	words := strings.Split(sillyProfanity, " ")
